      ...
      <section className="relative overflow-hidden bg-navy-900 pb-20 pt-36 text-white">
        ...
        <div className="relative mx-auto max-w-6xl px-5 sm:px-8">
          <Reveal>
            ...
            <Lead dark>
              Catalyst Innovations is building a portfolio of specialized
              products across business functions. Every entry below is labeled
              by its honest development status — we never present a concept as a
              deployed product.
            </Lead>
            <div className="mt-8 flex flex-wrap gap-3 text-xs text-ice-300">
              <span className="rounded-full border border-steel-400/40 bg-steel-400/10 px-3 py-1.5">
                In Development — actively being built
              </span>
              <span className="rounded-full border border-warning/40 bg-warning/10 px-3 py-1.5">
                Concept — validated direction, not yet built
              </span>
              <span className="rounded-full border border-white/20 bg-white/5 px-3 py-1.5">
                Client-Specific — built for individual engagements
              </span>
            </div>
          </Reveal>
        </div>
      ...
      <Section className="bg-ice-50">
          ...
          {products.map((p) => (
            <RevealItem key={p.slug} className="h-full">
              <article className="flex h-full flex-col rounded-card border border-ice-200 bg-white p-8 shadow-card">
                ...
                <p className="mt-1 text-xs font-medium tracking-wide text-navy-700 uppercase">
                  For: {p.audience}
                </p>
        ...
        <Reveal className="mt-14 rounded-card border border-dashed border-ice-300 bg-white/60 p-8 text-center">
          ...
          <p className="mx-auto mt-2 max-w-lg text-sm text-navy-700">
            We publish results only when they&apos;re real and verified. Early
            implementation partners will be featured here with their permission.
          </p>
        </Reveal>
      </Section>
      <CTABand
        title="Have a workflow that could work better?"
        body="Explore an illustrative demo, then talk with us about a clearly scoped solution for your business."
      />
    </>
  );
}
