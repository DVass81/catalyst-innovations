          ...
          {industries.map((i) => {
            ...
            return (
              <button
                ...
                onKeyDown={(e) => {
                  const index = industries.findIndex((x) => x.slug === active);
                  let n = index;
                  if (e.key === "ArrowDown" || e.key === "ArrowRight")
                    n = (index + 1) % industries.length;
                  else if (e.key === "ArrowUp" || e.key === "ArrowLeft")
                    n = (index + industries.length - 1) % industries.length;
                  else if (e.key === "Home") n = 0;
                  else if (e.key === "End") n = industries.length - 1;
                  else return;
                  e.preventDefault();
                  setActive(industries[n].slug);
                  document.getElementById(industries[n].slug)?.focus();
                }}
                aria-controls="industry-panel"
                onClick={() => {
                  setActive(i.slug);
                  track("demo_interaction", {
                    widget: "industry_explorer",
                    industry: i.slug,
                  });
                }}
              ...
              >
                <Icon
                  name={i.icon}
                  size={18}
                  className={selected ? "text-steel-300" : "text-steel-600"}
                />
                {i.name}
              </button>
            );
            ...
            >
              ...
              <div className="mt-6 grid gap-3 rounded-card border border-ice-200 bg-ice-50 p-1.5 sm:grid-cols-2">
                <div className="rounded-[10px] bg-white p-4">
                  <p className="flex items-center gap-1.5 text-xs font-bold tracking-[0.18em] text-navy-700 uppercase">
                    <XCircle size={13} className="text-danger/70" /> The old way
                  </p>
                  <p className="mt-2 text-sm leading-snug text-navy-700">
                    {current.problems[0]}
                  </p>
                </div>
                <div className="rounded-[10px] bg-navy-900 p-4">
                  <p className="flex items-center gap-1.5 text-xs font-bold tracking-[0.18em] text-steel-300 uppercase">
                    <CheckCircle2 size={13} className="text-success" /> With
                    Catalyst
                  </p>
              ...
              <div className="mt-8 grid gap-8 md:grid-cols-3">
                <div>
                  <h3 className="flex items-center gap-2 font-display text-sm font-semibold text-navy-900">
                    <AlertCircle size={16} className="text-warning" /> Common
                    problems
                  </h3>
                  <ul className="mt-3 space-y-2.5">
                    {current.problems.map((p) => (
                      <li
                        key={p}
                        className="text-sm leading-snug text-navy-700"
                      >
                        {p}
                      </li>
                    ))}
                ...
                <div>
                  <h3 className="flex items-center gap-2 font-display text-sm font-semibold text-navy-900">
                    <CheckCircle2 size={16} className="text-steel-500" /> How we
                    can help
                  </h3>
                  <ul className="mt-3 space-y-2.5">
                ...
                <div>
                  <h3 className="flex items-center gap-2 font-display text-sm font-semibold text-navy-900">
                    <Target size={16} className="text-success" /> Target
                    outcomes
                  </h3>
                  <ul className="mt-3 space-y-2.5">
              ...
              <Link
                href="/consultation"
                onClick={() =>
                  track("cta_consultation_click", {
                    location: "industry_explorer",
                  })
                }
